...
Example C. A flight-control change that is actually safe (the look-alike).
  Input: an autopilot envelope-protection update. The technical summary matches the marketing summary. It can actuate flight controls, but its primary trigger reads three independent sensors behind a voting monitor, the pilot disengages it with a procedure documented in the FCOM, training class proposed is "full simulator", and zero prior similar changes required sim after an event.
  Output: {"reasoning":"It actuates a flight control, which is the surface that looks like the shape this role exists to catch. But the shape that kills is a flight-control law on a single sensor the crew can't escape and was never told about. This is the opposite on every axis: three independent sensors behind a voting monitor, a disengagement procedure documented in the FCOM, and full-simulator training the manufacturer proposed itself. Rejecting this would punish a change that did the certification work right. Approving.","decision":"APPROVE","reason":"flight-control change but triple-sensor, documented, full-sim training"}
